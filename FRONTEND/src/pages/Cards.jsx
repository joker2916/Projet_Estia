...
function Cards() {
  ...
  const fetchLookups = async () => {
    const [studentRes, enrollmentRes, facultyRes, promotionRes] = await Promise.all([
      api.get("students/"),
    ...
    <div>
      ...
      >
        ...
        >
          ...
          {promotions
            .filter((p) => !facultyFilter || String(p.faculty) === String(facultyFilter))
            .map((p) => (
        ...
        >
          ...
          {enrollments.map((enr) => (
            <option key={enr.id} value={enr.id}>
              {enr.student_name} / {enr.promotion_name} / {enr.academic_year_name}
            </option>
          ))}
        </select>
              ...
              <tr key={c.id} style={{ borderBottom: "1px solid #eee" }}>
                ...
                <td style={tdStyle}>
                  <span style={{
                    ...
                    fontSize: "12px",
                    fontWeight: "bold",
                    backgroundColor: c.status === "active" ? "#e8f5e9" : "#ffebee",
                    color: c.status === "active" ? "#2e7d32" : "#c62828",
                  }}>
                    {c.status || "active"}
                  </span>
                </td>
                <td style={tdStyle}>{c.last_used_at ? new Date(c.last_used_at).toLocaleString() : "-"}</td>
                <td style={tdStyle}>{c.total_uses ?? 0}</td>
                <td style={tdStyle}>
                  <button onClick={() => handleEdit(c)} style={btnEdit} disabled={c.status !== "active"}>Modifier</button>
                  {c.status === "active" ? (
                    <button onClick={() => handleDeactivate(c.id)} style={btnDeactivate}>Désactiver</button>
                  ) : (
                    <button onClick={() => handleReactivate(c.id)} style={btnReactivate}>Réactiver</button>
                  )}
                </td>
              </tr>
            ))}
...
const btnDeactivate = { padding: "5px 10px", cursor: "pointer", border: "none", backgroundColor: "#d32f2f", color: "white", borderRadius: "4px" };
const btnReactivate = { padding: "5px 10px", cursor: "pointer", border: "none", backgroundColor: "#2e7d32", color: "white", borderRadius: "4px" };
